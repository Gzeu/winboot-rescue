...
function Write-Log {
    ...
    .PARAMETER Level    INFO (default), WARN, ERROR, SUCCESS, STEP, RAW, SECTION
...
function Invoke-LoggedCommand {
    <#
    .SYNOPSIS Executes a command, captures all output, logs it, and returns results.
...
function Collect-Diagnostics {
    ...
    try {
        ...
        Write-Log "CIM logical disk query failed: $_" -Level WARN
...
function Repair-UefiBoot {
    ...
    .NOTES This is the correct approach for UEFI systems. Do NOT use bootrec /fixboot as primary.
    #>
    ...
    if (-not $efiInfo.Found) {
        ...
        Write-Log "  -> If you have a backup, restore it. Otherwise consider creating a new EFI partition." -Level ERROR
        return $false
    }
    ...
    Write-Log "  because it targets the MBR/VBR — which doesn't exist in pure UEFI mode." -Level INFO
...
function Repair-BiosBoot {
    ...
    Write-Log "Step 2/5: Fixing MBR with bootrec /fixmbr..." -Level STEP
    Write-Log "  This rewrites the Master Boot Record. Safe — does not affect partitions." -Level INFO
    $mbrResult = Invoke-LoggedCommand -Command 'bootrec' -Arguments @('/fixmbr') `
        -Description "bootrec /fixmbr" -SaveRawAs "bootrec_fixmbr.txt" -IgnoreExit
    if (-not $mbrResult.Success) {
        Write-Log "bootrec /fixmbr failed (exit $($mbrResult.ExitCode))" -Level WARN
        $allSuccess = $false
    }

    Write-Log "Step 3/5: Fixing Volume Boot Record with bootrec /fixboot..." -Level STEP
    ...
    if ($fixbootOutput -match 'Access is denied') {
        Write-Log "bootrec /fixboot returned 'Access is denied'." -Level WARN
        Write-Log "  This usually happens when:" -Level INFO
        Write-Log "  1. The system is actually UEFI (use Repair-UefiBoot instead)" -Level INFO
    ...
    Write-Log "  This scans all disks and adds found Windows installations to BCD." -Level INFO
    Write-Log "  NOTE: This is an interactive command — it will prompt you to add entries." -Level WARN

    Write-Host "`n[ACTION REQUIRED] bootrec /rebuildbcd will now run interactively." -ForegroundColor Yellow
    Write-Host "  Answer 'Y' when prompted to add the found Windows installation." -ForegroundColor Yellow
    Write-Host "  Press any key to continue..." -ForegroundColor Yellow
    $null = $Host.UI.RawUI.ReadKey("NoEcho,IncludeKeyDown")

    try {
        $reBcdOutput = & bootrec.exe /rebuildbcd 2>&1
        $reBcdExit   = $LASTEXITCODE
        $reBcdStr    = $reBcdOutput -join "`n"
        Write-Log "bootrec /rebuildbcd output:" -Level INFO
        $reBcdOutput | ForEach-Object { Write-Host "  $_" -ForegroundColor DarkGray }
        Save-RawOutput -FileName "bootrec_rebuildbcd.txt" -Content $reBcdStr

        if ($reBcdExit -ne 0 -or $reBcdStr -match 'The requested system device cannot be found') {
            Write-Log "bootrec /rebuildbcd had issues (exit $reBcdExit). Trying bcdboot fallback..." -Level WARN

            Write-Log "Fallback: bcdboot $winDir /f BIOS" -Level STEP
            $bcdbootBios = Invoke-LoggedCommand -Command 'bcdboot' `
                -Arguments @($winDir, '/f', 'BIOS') `
                -Description "bcdboot BIOS (fallback after failed rebuildbcd)" -IgnoreExit
            if ($bcdbootBios.Success) {
                Write-Log "bcdboot BIOS fallback succeeded." -Level SUCCESS
            } else {
                Write-Log "bcdboot BIOS fallback also failed. System may need manual recovery." -Level ERROR
                $allSuccess = $false
            }
        } else {
            Write-Log "bootrec /rebuildbcd succeeded." -Level SUCCESS
        }
    } catch {
        Write-Log "Exception running bootrec /rebuildbcd: $_" -Level ERROR
        $allSuccess = $false
    }

    if ($allSuccess) {
        Write-Log "BIOS/MBR boot repair completed successfully." -Level SUCCESS
        $Script:RepairDone = $true
    } else {
        Write-Log "BIOS/MBR boot repair completed with some warnings/errors. Review logs." -Level WARN
        $Script:RepairDone = $true
    }
...
function Run-Chkdsk {
    ...
    .NOTES In WinPE/WinRE, chkdsk can run on offline volumes without scheduling.
    #>
    ...
    Write-Host "`n[INFO] CHKDSK will scan $driveLetter for file system errors." -ForegroundColor Cyan
    Write-Host "       This may take several minutes on large disks." -ForegroundColor Cyan
    Write-Host "       /F = fix errors | /R = locate bad sectors (SLOW)" -ForegroundColor Cyan
    Write-Host ""
    ...
    switch ($result.ExitCode) {
        ...
        1 { Write-Log "CHKDSK: Errors were found and fixed." -Level WARN }
        2 { Write-Log "CHKDSK: Disk cleanup recommended." -Level WARN }
